...
class Spider(Spider):  # 元类 默认的元类 type
	...
	def homeVideoContent(self):
		rsp = self.fetch("https://www.baipiaoys.cc")
	...
	def categoryContent(self,tid,pg,filter,extend):
		...
		suffix = '/'.join(params)
		url = 'https://www.baipiaoys.cc/show/{0}.html'.format(suffix)
		rsp = self.fetch(url)
		root = self.html(rsp.text)
		aList = root.xpath("//div[@class='stui-vodlist__box']/a")
	...
	def detailContent(self,array):
		tid = array[0]
		url = 'https://www.baipiaoys.cc/detail/{0}.html'.format(tid)
		rsp = self.fetch(url)
		root = self.html(rsp.text)
	...
	def searchContent(self,key,quick):
		url = 'https://www.baipiaoys.cc/search.html?wd={0}'.format(key)
		rsp = self.fetch(url)
		root = self.html(rsp.text)
	...
	header = {
		#"Referer":"https://www.baipiaoys.cc/",
		"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/94.0.4606.54 Safari/537.36"
	}
	def playerContent(self,flag,id,vipFlags):
		url = 'https://www.baipiaoys.cc/play/{0}.html'.format(id)
		rsp = self.fetch(url)
