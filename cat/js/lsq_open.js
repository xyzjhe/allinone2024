...
function getCssVal($, item, parse) {
    ...
    if (item) {
        if(xpa[1] == 'Text') {
            ...
            if(xpa[0]) {
                val = $(item).find(xpa[0]).attr(xpa[1]);
            } else {
                val = $(item).attr(xpa[1]);
            }
        ...
        if(xpa[1] == 'Text') {
            ...
            val = $(xpa[0]).attr(xpa[1]);
        }
    }
    
    if(xpa[2]) {
        val = val.match(new RegExp(xpa[2]))[1];
    }
    return val;
}
...
function getCssValArray($, item, parse) {
    ...
    _.forEach(el, item => {
        ...
        if(xpa[2]) v = v.match(new RegExp(xpa[2]))[1];
        if (v) {
            val.push(v);
        }
...
async function detail(id) {
    ...
    if (rule.二级) {
        ...
        if ('object' === typeof(parse)) {
            ...
            if (parse['tabs'] && parse['lists']) {
                const playMap = {};
                const tabNames = getCssValArray($, '', parse['tabs']);
                const lists = $(parse['lists']);
                _.each(lists, (list,i) => {
                    //console.log('tab_exclude', parse['tab_exclude'].indexOf(tabNames[i]));
                    if(rule['tab_exclude'] && rule['tab_exclude'].indexOf(tabNames[i]) > -1) {
                        return;
                    }
                    _.each($(list).find('a'), it => {
                        const title = getCssVal($, it, parse['list_text']);
                        const playUrl = getCssVal($, it, parse['list_url']);
